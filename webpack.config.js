          ...
          if (isFirefox) {
            ...
            if (value.permissions) {
              value.permissions = value.permissions.filter(
                p => !['declarativeNetRequest', 'declarativeNetRequestWithHostAccess', 'sidePanel', 'background'].includes(p)
              );
            ...
            if (value.content_security_policy) {
              const csp = value.content_security_policy;
              if (typeof csp === 'object') {
                const scriptSrc = csp['script-src'] || "'self'";
                // 移除 Firefox 不支持的 'unsafe-eval'
