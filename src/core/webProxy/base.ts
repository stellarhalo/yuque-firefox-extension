...
class HttpProxy {
  ...
  sendMethodCallToBackground<T>(
    ...
    if (Env.isExtensionPage && !Env.isFirefox) {
    ...
    if (type !== 'abort') {
      this.callServiceMethodCallbackFn[callbackFnId] = (response: any) => {
        const r = response?.response !== undefined ? response.response : response;
        callback?.(r);
      };
    }
    ExtensionMessage.sendToBackground(
      ...
      if (response && typeof response === 'object' && response.funcId) {
        const fn = this.callServiceMethodCallbackFn[response.funcId];
        if (fn) fn(response);
      } else if (callback && response !== undefined) {
        callback(response);
      }
    });
  }
