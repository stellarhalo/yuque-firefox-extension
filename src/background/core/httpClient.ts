...
export default class HttpClient {
  ...
  constructor() {
    console.log('[FF] HttpClient constructor, isBackground:', Env.isBackground);
    // 只在后台监听，其余系统页都不监听
    if (!Env.isBackground) {
      return;
    }
    console.log('[FF] HttpClient registering message listener');
    // 监听关闭的请求
    ExtensionMessage.addListener(ExtensionMessageListener.callRequestService, async (data, requestId, { tab }, sendResponse) => {
      console.log('[FF] HttpClient received request:', data?.type, 'callbackFnId:', data?.callbackFnId);
      const { type, methodParams = {}, callbackFnId } = data;

      const callback = (response: any) => {
        console.log('[FF] HttpClient sending response for callbackFnId:', callbackFnId);
        try {
          if (callbackFnId) {
            sendResponse({
              funcId: callbackFnId,
          ...
          console.error('[FF] sendResponse error:', e);
        }
      };
  ...
  async handleRequest(
    ...
    try {
      ...
      console.error('[FF] HttpClient error:', e?.status, e?.message || e);
      // 登录过期
